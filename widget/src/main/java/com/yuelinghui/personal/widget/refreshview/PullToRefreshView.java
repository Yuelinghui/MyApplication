...
public class PullToRefreshView extends ViewGroup{
    ...
    public static final int QSTYLE_SUN = 1;
    public static final int QSTYLE_MOON = 2;
    ...
    public PullToRefreshView(Context context, AttributeSet attrs) {
        ...
        refreshEnable = a.getBoolean(R.styleable.RefreshView_refresh_enable, true);
        a.recycle();

        mDecelerateInterpolator = new DecelerateInterpolator(DECELERATE_INTERPOLATION_FACTOR);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        mTotalDragDistance = LocalDisplay.dp2px(DRAG_MAX_DISTANCE);

        if (refreshEnable) {
            mRefreshView = new ImageView(context);
            ViewGroup.LayoutParams lp = new LayoutParams(-1, -2);
    ...
    public void setRefreshStyle(int type) {
        ...
        switch (type) {
            case STYLE_SUN:
                mBaseRefreshView = new HeaderRefreshView(getContext(), this);
                break;
            case QSTYLE_SUN:
                mBaseRefreshView = new HeaderRefreshView(getContext(), this);
                mRefreshView.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.common_bg));
                break;
            case QSTYLE_MOON:
                mRefreshView.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.night_mode_decoration));
                mBaseRefreshView = new HeaderRefreshView(getContext(), this, true);
                break;
            default:
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || !refreshEnable || canChildScrollUp() || mRefreshing) {
            return false;
        }
    ...
    public boolean onTouchEvent(@NonNull MotionEvent ev) {
        ...
        if (!mIsBeingDragged || !refreshEnable) {
    ...
    public void setPullDownRefreshEnable() {
        setEnabled(true);
    }

    private void setRefreshing(boolean refreshing, final boolean notify) {
        if (mRefreshing != refreshing && refreshEnable) {
            mNotify = notify;
            ensureTarget();
